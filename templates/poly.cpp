...
#define gb(x) (b->get((x) + ob))
                ...
                if (u == 0)
                    ...
                    cp[u] += sa[0] * lb[u & 127] + la[u & 127] * sb[0];
                if ((cm + 1) * 2 <= u)
                {
                    int k = getK((u + 1) * 2);
                    ...
                    if (cs > 16)
                    {
                        ...
                        for (int i = 0; i < c; ++i)
                        {
                            ...
                            if (!is_sqr)
                            {
                                for (int j = 0; j < cs; ++j)
                                    tg[j] +=
                                        cur.first.first[j] * (fbd.second[j] + fbd2.second[j]) + cur.first.second[j] * (fbd.first[j] + fbd2.first[j]);
                        ...
                        if (!is_sqr)
                        {
                            ...
                            case 8:
                                ...
                                for (int i = u + 1; i <= r; ++i)
                                {
                                    ...
                                    {
#define par(s) \
    (ull) pa[j + s].w *sb[i - j - s].w + (ull)pb[j + s].w *sa[i - j - s].w
                                        sum += par(0) + par(1) + par(2) + par(3) + par(4) + par(5) + par(6) + par(7);
                                        sum %= MOD;
#undef par
                                    }
                                    cp[i] += int(sum);
                                ...
                                break;
#define par(s) \
    ((ull)pa[l + s].w * sb[i - l - s].w + (ull)pb[l + s].w * sa[i - l - s].w)
                            case 12:
                                for (int i = u + 1; i <= r; ++i)
                                    cp[i] += int(((par(0) + par(1) + par(2) + par(3) + par(4) + par(5) + par(6) + par(7)) % MOD + par(8) + par(9) + par(10) + par(11)) % MOD);
                                break;
#undef par
                            default:
                                for (int i = u + 1; i <= r; ++i)
...
#define par(s) \
    (ull) pa[j + s].w *sa[i - j - s].w
                                        sum += par(0) + par(1) + par(2) + par(3) + par(4) + par(5) + par(6) + par(7) + par(8) + par(9) + par(10) + par(11) + par(12) + par(13) + par(14) + par(15);
                                        sum %= MOD;
#undef par
                                    }
                                    cp[i] += int(sum) * 2 % MOD;
...
#define gb(x) (b->get((x) + ob))
                ...
                if (u < 128)
                    sa[u] = ga(u);
                if (u < 64)
                    sb[u * 2] = gb(u * 2), sb[u * 2 + 1] = gb(u * 2 + 1);
                cp[u] += la[u & 127] * sb[0];
                auto gfa = [&](int cu, int of, int cs)
                {
                    int id = cu * 8 + of;
